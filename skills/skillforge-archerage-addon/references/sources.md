...
## 2026-09-13 r3：Native事件与跑商状态机取材

为纠正本轮实际出现的“把变化量当余额”“把冷却当响应超时”，额外静态阅读 Noviern/aagamedump（提交 `05b989aa52678b7332502c44b7fa37c63e133ab5`，页面标记2025-10-29）的客户端UI实现：

| 文件 | 本次只采纳的语义 | 不外推的内容 |
|---|---|---|
| `globalui/chat/chat_msg_event.lua` | `PLAYER_MONEY(change, changeStr, itemTaskType, info)`；Honor/Living参数按变化量生成聊天提示 | 其中调用的余额/点数Getter不因此自动成为当前RU插件Allowed |
| `x2ui/hud/main_menu_bar/exp_bar_set.lua` | `EXP_CHANGED(stringId, expNum, expStr)`用于刷新玩家经验条 | `GetExpInfo/GetHeirExpInfo`在客户端内部可用不等于插件门禁允许 |
| `x2ui/combattext/combat_text.lua` | 玩家UnitId匹配后把EXP_CHANGED的经验量显示为GAIN_EXP | 不把宠物/其他单位经验事件算玩家收益 |
| `x2ui/specialty/info.lua` | `GetSpecialtyRatioBetween`返回值用于搜索按钮冷却；货率由 `SPECIALTY_RATIO_BETWEEN_INFO` 异步填表 | 不能从该实现推导当前RU固定SLA或重试次数 |
| `commonui/logic/money.lua` | 金银铜字符串按最小单位切分的显示规则 | 仅为格式语义，不授权任何余额Getter |

这些公开客户端源码是**语义参考**，不是当前ArcheRage RU插件权限清单。当前项目本地 `z_api_functions` / Capability 分类仍优先；用户现场诊断用于证明事件确实出现和旧算法的可见错误。r3没有执行RU客户端或独立Agent A/B。
