...
class R3RegressionLessonsTests(unittest.TestCase):
    def test_recent_runtime_failures_are_distilled_into_r3(self):
        skill=(ROOT/'SKILL.md').read_text(encoding='utf-8')
        lessons=(ROOT/'references/maintenance-lessons-2026-09.md').read_text(encoding='utf-8')
        runtime=(ROOT/'references/runtime-and-api.md').read_text(encoding='utf-8')
        ...
        rows=json.loads((ROOT/'evals/behavior-cases.json').read_text(encoding='utf-8'))
        ids={row['id'] for row in rows}
        for case in range(53,61):
            self.assertIn(f'AR-{case}', ids)
