...
| 金币/荣誉/经验数值不对、事件参数含义不明 | [运行时与API](references/runtime-and-api.md)、[服务器日期账本](references/server-day-ledger.md) |
...
| 查询一直“正在进行”、冷却/超时混淆 | [回调与请求契约](references/callback-and-request-contracts.md) |
| 正式页正常但今日总览/嵌入卡片没数据 | [UI与生命周期](references/ui-and-lifecycle.md)、[回调与请求契约](references/callback-and-request-contracts.md) |
