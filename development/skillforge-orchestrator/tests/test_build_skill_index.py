...
SCRIPT = Path(__file__).resolve().parents[1] / "tools" / "build_skill_index.py"
...
class IndexTests(unittest.TestCase):
    ...
    def test_root_is_required_no_sibling_assumption(self):
        self.assertTrue(SCRIPT.is_file())
        q = subprocess.run([sys.executable, "-B", str(SCRIPT)], capture_output=True, text=True, encoding="utf-8", timeout=10, env={**os.environ,"PYTHONDONTWRITEBYTECODE":"1"})
        self.assertNotEqual(q.returncode, 0)
        self.assertIn("--root", q.stderr)
